...
from ..utils import mask_phone, mask_name, should_hide_sensitive, format_money, parse_date

console = Console()


@click.group(help="记录催缴沟通：电话登记、承诺付款、减免记录")
def record_cmd():
    pass
...
@click.option("--arrear-id", type=int, help="关联欠费ID（可选，默认关联该住户最早的未缴欠费）")
@click.option("--contact", "-c", help="通话对象")
@click.option("--call-time", help="通话时间，格式YYYY-MM-DD HH:MM，默认当前时间")
@click.option("--result", "-s", required=True, type=click.Choice([
    "已联系/承诺付款", "已联系/待考虑", "已联系/拒缴",
    "已联系/停机空号", "无人接听", "关机",
    "号码错误", "其他"
]), help="通话结果")
@click.option("--commitment-date", help="承诺付款日期，格式YYYY-MM-DD")
@click.option("--commitment-amount", type=float, help="承诺付款金额")
@click.option("--remark", "-m", help="备注说明")
def record_call(room_no, arrear_id, contact, call_time, result, commitment_date, commitment_amount, remark):
    ...
    if arrear_id:
        ar = conn.execute("SELECT * FROM arrears WHERE id = ? AND household_id = ?",
                          (arrear_id, household_id)).fetchone()
        if not ar:
            console.print(f"[red]欠费ID {arrear_id} 不存在或不属于该住户[/red]")
            conn.close()
            return
    else:
        ar = conn.execute("""
            SELECT * FROM arrears
            WHERE household_id = ? AND (total_amount - paid_amount - discount_amount) > 0
...
[bold]关联欠费:[/bold] {f'ID={ar["id"]} {ar["fee_period"]} {format_money(ar["total_amount"]-ar["paid_amount"]-ar["discount_amount"])}元' if ar else '无'}
...
"""
    ...
    try:
        ...
        if result == "已联系/承诺付款" and arrear_id_val:
            conn.execute("""
                UPDATE arrears SET status = '承诺付款', updated_at = datetime('now','localtime')
                WHERE id = ?
            """, (arrear_id_val,))
...
@click.option("--has-commitment", is_flag=True, help="仅显示有承诺付款的记录")
...
@click.option("--date", "commit_date", required=True, help="承诺付款日期 YYYY-MM-DD")
@click.option("--amount", type=float, help="承诺付款金额，默认全额")
...
def mark_commitment(arrear_id, commit_date, amount, remark):
    ...
    hide = should_hide_sensitive()
    unpaid = ar["total_amount"] - ar["paid_amount"] - ar["discount_amount"]
    ...
    if not click.confirm("确认标记为承诺付款？"):
        conn.close()
        return
    ...
    try:
        ...
        conn.commit()
        console.print("[green]已标记为承诺付款[/green]")
    except Exception as e:
        conn.rollback()
...
def record_discount(arrear_id, amount, reason, approved_by, remark):
    ...
    conn = get_connection()
    ar = conn.execute("""
        SELECT a.*, h.room_no, h.owner_name
        FROM arrears a JOIN households h ON a.household_id = h.id
        WHERE a.id = ?
    """, (arrear_id,)).fetchone()
    if not ar:
    ...
    if amount > unpaid + 0.01:
        console.print(f"[red]减免金额({format_money(amount)})不能超过尚欠金额({format_money(unpaid)})[/red]")
        if not click.confirm("是否继续？（将减免至0）"):
            conn.close()
            return
        amount = unpaid

    display_name = mask_name(ar["owner_name"]) if hide else ar["owner_name"]
    new_discount = ar["discount_amount"] + amount
    new_total = ar["base_amount"] + ar["late_fee"] - new_discount
    new_unpaid = new_total - ar["paid_amount"]
    new_status = "已缴" if new_unpaid <= 0.01 else ("减免" if amount == unpaid else ar["status"])
...
[yellow]累计减免:[/yellow] {format_money(new_discount)}元
[green]减免后尚欠:[/green] {format_money(max(new_unpaid, 0))}元  [bold]状态将变为: {new_status}[/bold]
...
"""
    ...
    try:
        ...
        conn.execute("""
            ...
            (arrear_id, household_id, discount_amount, discount_reason, approved_by, approved_at)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (arrear_id, ar["household_id"], amount, reason, approved_by, approved_at))

        conn.execute("""
            UPDATE arrears SET
                discount_amount = discount_amount + ?,
                total_amount = base_amount + late_fee - (discount_amount + ?),
                status = ?,
                updated_at = datetime('now','localtime')
            WHERE id = ?
        """, (amount, amount, new_status, arrear_id))

        conn.commit()
        console.print(f"[green]已登记减免 {format_money(amount)} 元，状态: {new_status}[/green]")
    except Exception as e:
        conn.rollback()
        console.print(f"[red]登记失败: {str(e)}[/red]")
    finally:
...
@record_cmd.command("payment", help="登记实际缴费")
@click.argument("arrear_id", type=int)
...
@click.option("--remark", help="备注")
def record_payment(arrear_id, amount, pay_date, method, remark):
    if amount <= 0:
    ...
    unpaid = ar["total_amount"] - ar["paid_amount"] - ar["discount_amount"]
    if amount > unpaid + 0.01:
        console.print(f"[red]缴费金额({format_money(amount)})超过尚欠({format_money(unpaid)})[/red]")
        if not click.confirm("是否按全额缴纳处理？"):
            conn.close()
            return
        amount = unpaid
    ...
    new_unpaid = ar["total_amount"] - new_paid - ar["discount_amount"]
...
[magenta]业主:[/magenta] {display_name}  [white]账期:[/white] {ar['fee_period']}
[blue]原尚欠:[/blue] {format_money(unpaid)}元
[bold green]本次缴纳:[/bold green] {format_money(amount)}元  [dim]方式: {method} 日期: {actual_pay_date}[/dim]
[yellow]累计已缴:[/yellow] {format_money(new_paid)}元
[bold]缴后尚欠: {format_money(max(new_unpaid, 0))}元  状态: {new_status}[/bold]
    ...
    try:
        conn.execute("""
            UPDATE arrears SET
                ...
                updated_at = datetime('now','localtime')
            WHERE id = ?
        """, (amount, new_status, remark or None, arrear_id))
        conn.commit()
        console.print(f"[green]已登记缴费 {format_money(amount)} 元，状态: {new_status}[/green]")
    except Exception as e:
        conn.rollback()
        console.print(f"[red]登记失败: {str(e)}[/red]")
    finally:
